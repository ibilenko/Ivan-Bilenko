...
from telebot.types import Label
...
class main:
    ...
    ask_promo = False  # при True ож    идает промокод
...
Вам подарок 🎁 - """
    ...
    def append_user(user_id: int, engine) -> None:
        ...
        engine.connect().execute(
            f"""insert into gogol.users values ({user_id},0, 0,'Лёгкий', '{now}' ,null,2500,'menu')""")
        main.new_user = False
    ...
    def get_menu_info(user_id: int, engine) -> None:
        ...
        df = pd.read_sql(
            f"""select spend_symbols, allowed_symbols, algoritm, mode from gogol.users where id = {user_id}""",
            engine)
        if not df.empty:
...
Что будем делать?"""
    ...
    def start_message(message):
        main.new_line('gogol_logs.txt', str(message.chat.id)+ ' '+ str(datetime.now()))
        ...
        main.check_user(user_id, main.engine)
        if main.new_user:
            main.new_line('gogol_logs.txt', str(message.chat.id) + str(' оказался новым юзером'))
            bot.send_message(user_id, main.start_txt)
            main.ask_promo = True
            main.check_promo = True
            bot.send_message(user_id, text=main.promo_ask, reply_markup=kb)
            main.append_user(user_id, main.engine)
            main.new_user = False
        else:
            main.new_line('gogol_logs.txt', str(message.chat.id) + str(' юзер уже есть в базе'))
    ...
    def send_screen(message):
        ...
        if main.check_bot_status(user_id, main.engine):
            ...
            if message.text == 'Меню' and not main.new_user:
                main.get_menu_info(user_id, main.engine)
                bot.send_message(user_id, text=main.menu_txt(), reply_markup=main.kb_menu,
                                 parse_mode=telegram.ParseMode.MARKDOWN)
            ...
            if main.bot_working and not main.prohibition:  # (1) здесь он принимает текст от пользователя
                ...
                if len_symbols > 3800:
                    ...
                    bot.send_message(user_id,
                                     text=f'Слишком длинный для обработки текст. Попробуй уменьшить и отправить снова 🙃',
                                     reply_markup=main.kb_zero_symbols,
                                     parse_mode=telegram.ParseMode.MARKDOWN)
                    ...
                    if main.allowed_symbols != -1 and main.allowed_symbols - len_symbols < 0:
                        ...
                        main.get_menu_info(user_id, main.engine)
                        try:
                            result = gogolt.send_text(message.text, main.alg, user_id)
                        except:
                            result = 'Ой, где-то что-то пошло не так. Попробуй снова'
                            
                        bot.send_message(user_id, text=result, reply_markup=main.kb_algo,
                                         parse_mode=telegram.ParseMode.MARKDOWN)
                        main.get_menu_info(user_id, main.engine)

                        if result != 'Ой, где-то что-то пошло не так. Попробуй снова':
        ...
        if main.check_bot_status(user_id, main.engine):
            if call.data == 'change_alg_working_mode':
                if main.alg == main.alg_hard:
                    ...
                    bot.send_message(user_id, text=f'Поменял алгоритм на {main.alg_easy}. Введите текст:',
                                     parse_mode=telegram.ParseMode.MARKDOWN)
                else:
                    main.alg = main.alg_hard
                    main.change_algoritm(user_id, main.engine, "Сложный")
                    bot.send_message(user_id, text=f'Поменял алгоритм на {main.alg_hard}. Введите текст:',
                                     parse_mode=telegram.ParseMode.MARKDOWN)
            if call.data == 'menu':
                main.get_menu_info(user_id, main.engine)
                bot.send_message(user_id, text=main.menu_txt(), reply_markup=main.kb_menu,
            ...
            if call.data == 'balance':
                bot.send_message(user_id, text=main.balance_txt, reply_markup=main.kb_balance,
                                 parse_mode=telegram.ParseMode.MARKDOWN)
                main.bot_working = False
            ...
            if call.data == "begin":  # эта кнопка начать, по которой он включает режим ожидания текста (1)
                ...
                bot.send_message(user_id, text=main.begin_txt, reply_markup=main.markup,
                                 parse_mode=telegram.ParseMode.MARKDOWN)
            if call.data == "manual":
                ...
                main.bot_working = False
            if call.data == 'first':
                bot.send_invoice(user_id, title='Пакет "Курсач"', start_parameter=None,
            ...
            if call.data == 'second':
                bot.send_invoice(user_id, title='Пакет "Диплом"', start_parameter=None,
            ...
            if call.data == 'third':
                bot.send_invoice(user_id, title='Пакет "Писатель"', start_parameter=None,
